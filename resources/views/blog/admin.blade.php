      ...
      <header class="app-header">
        <nav class="navbar navbar-expand-lg navbar-light">
          <ul class="navbar-nav">
           ...
           <p style="padding-left:850px;">{{ Auth::user()->name }} <svg xmlns="http://www.w3.org/2000/svg" width="20" height="16" fill="currentColor" class="bi bi-person" viewBox="0 0 16 16">
          ...
          <div class="card">
            ...
            <div class="bg-light card-body">
              ...
              <form action="search" class="d-flex" role="search">
                <a href="{{ route('blog.create') }}" class="btn btn-md btn-success mb-3">Tambah</a>
                ...
                <tbody>
                  ...
                  <tr>
                    <td class="text-center">
                      <img src="{{ asset('/storage/blog/'.$blog1->image) }}" class="rounded" style="width:50px">
                    </td>
                    <td style="font-size:12px;">{{ $blog1->title }}</td>
                    <td style="font-size:12px;">{!! $blog1->content !!}</td>
                    <td class="text-center">
                      <img src="{{ asset('/storage/blog/'.$blog1->example) }}" class="rounded" style="width:50px">
                    </td>
                    <td class="col-3 text-center">

                      <form onsubmit="return 
confirm('Apakah Anda Yakin ?');" action="{{ route('blog.destroy', $blog1->id) }}" method="POST">
                        <a style="font-size:10px;" href="{{ route('blog.show', $blog1->id) }}" class="btn btn-sm btn-dark">SHOW</a>
                        <a style="font-size:10px;" href="{{ route('blog.edit', $blog1->id) }}" class="btn btn-sm btn-primary">EDIT</a>
                        @csrf
                        @method('DELETE')
